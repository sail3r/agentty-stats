...
static string human_bytes(long long n) {
  char buf[64];
  if (n < 1024)
    snprintf(buf, sizeof buf, "%lld B", n);
  else if (n < 1024 * 1024)
    snprintf(buf, sizeof buf, "%.1f KiB", n / 1024.0);
  else if (n < 1024LL * 1024 * 1024)
    snprintf(buf, sizeof buf, "%.2f MiB", n / (1024.0 * 1024.0));
  else
    snprintf(buf, sizeof buf, "%.2f GiB", n / (1024.0 * 1024.0 * 1024.0));
  return buf;
}
...
static string human_ms(long long ms) {
  char buf[64];
  if (ms < 1000)
    snprintf(buf, sizeof buf, "%lld ms", ms);
  else if (ms < 60000)
    snprintf(buf, sizeof buf, "%.1f s", ms / 1000.0);
  else
    snprintf(buf, sizeof buf, "%lldm %llds", ms / 60000, (ms % 60000) / 1000);
  return buf;
}
...
static long long parse_ts_ms(const string &ts) {
  ...
  if (sscanf(ts.c_str(), "%d-%d-%dT%d:%d:%d.%d", &y, &mo, &d, &h, &mi, &se,
             &ms) < 6)
    return 0;
  struct tm t;
  memset(&t, 0, sizeof t);
...
struct Turn {
  ...
  string role_inferred; // resolved role (route.turn's model mapped through
                        // router state)
...
struct Stats {
  ...
  map<string, long long>
      model_loaded_by_file;        // file -> count (last wins per file)
...
static void process_event(Stats &s) {
  ...
  if (ev == "route.turn") {
    ...
    if (t.provider.empty())
      t.provider = s.pend_provider; // falls back to last provider.select
    ...
    if (s.cur_turn) {
      ...
      if (s.cur_turn->model.empty())
        s.cur_turn->model = field(pl, "model");
      ...
      if (s.cur_turn->api_base.empty()) {
        ...
        if (p != string::npos) {
          ...
          if (std::regex_match(url, um, URL_RE)) {
            ...
            if (cc != string::npos)
              path = path.substr(0, cc);
            size_t cm = path.find("/completions");
            if (cm != string::npos)
              path = path.substr(0, cm);
    ...
    for (auto &sf : s.source_files)
      if (sf.path == s.cur_file) {
        sf.models_loaded = cnt;
        break;
      }
    ...
    if (s.error_turn_samples.size() < 8)
      s.error_turn_samples.push_back(s.cur_ts + " " + model + " " +
                                     field(pl, "msg"));
  } else if (ev == "stream.http_error") {
    string status = field(pl, "status");
    s.http_error_status[status]++;
    if (s.cur_turn)
      s.cur_turn->http_errors++;
    // per-model error attribution (turn model may still be unset)
    string model = s.cur_turn && !s.cur_turn->model.empty()
                       ? s.cur_turn->model
                       : field(pl, "model");
    if (!model.empty())
      s.models[model].errors++;
    if (s.error_turn_samples.size() < 8)
      s.error_turn_samples.push_back(
          s.cur_ts + " " + (model.empty() ? "?" : model) + " HTTP " + status);
    ...
    s.local_failures++;
    if (s.error_turn_samples.size() < 8)
      s.error_turn_samples.push_back(s.cur_ts + " conn_fail " +
                                     field(pl, "host"));
  ...
  if (is_error) {
    ...
    if (s.err_samples.size() < 8) {
      s.err_samples.push_back(s.cur_ts + " " + s.cur_comp + " " + ev + ": " +
                              pl);
    }
  }
}
...
static void bootstrap_roles(Stats &s, const char *path) {
  ...
  if (!f)
    return; // errors are reported by the main pass
  ...
  while (getline(&line, &cap, f) > 0) {
    ...
    if (!sline.empty() && sline.back() == '\n')
      sline.pop_back();
    if (sline.find("route.turn") == string::npos)
      continue;
    if (!std::regex_match(sline, m, ROUTE_RE))
      continue;
    string pl = m[1].str();
...
static bool process_file(Stats &s, const char *path) {
  ...
  s.cur_turn = nullptr;
  s.pend_role.clear();
  s.pend_complexity.clear();
  s.pend_model.clear();
  s.pend_effort.clear();
  s.pend_provider.clear();
  ...
  while ((nread = getline(&line, &cap, f)) != -1) {
    ...
    if (std::regex_match(ln, m, LINE_RE)) {
      ...
      if (wall > 0) {
        if (wall < s.wall_min_ms) {
          s.wall_min_ms = wall;
          s.first_ts = s.cur_ts;
        }
        if (wall > s.wall_max_ms) {
          s.wall_max_ms = wall;
          s.last_ts = s.cur_ts;
        }
      }
...
struct Pricing {
  ...
  map<string, ModelCost>
      by_id; // "provider-id\x1fmodel-key" + bare-alias entries
  map<string, bool>
      key_provider; // "model-key\x1fprovider-id": provider carries this model
...
static string norm_api(string a) {
  ...
  if (h != string::npos)
    a.erase(h);
  while (!a.empty() && isspace((unsigned char)a.back()))
    a.pop_back();
  while (!a.empty() && isspace((unsigned char)a.front()))
    a.erase(a.begin());
...
static string provider_id_for(const Pricing &pr, const string &label,
  ...
  if (pr.api_to_id.empty())
    return ""; // no pricing loaded
  if (!label.empty()) {
    string clean = norm_api(label);
    if (pr.id_name.count(clean))
      return clean; // exact id
    for (const auto &kv : pr.id_name)
      if (iequals(kv.second, clean))
        return kv.first; // display name
    auto it = pr.api_to_id.find(clean);
    if (it != pr.api_to_id.end())
      return it->second; // api URL
  }
  if (!api_base.empty()) {
    auto it2 = pr.api_to_id.find(norm_api(api_base));
    if (it2 != pr.api_to_id.end())
      return it2->second; // request-URL ground truth
  }
  return "";
}
...
static bool pricing_from_json(const string &buf, Pricing &pr) {
  ...
  if (!ps.parse_value(root, 0))
    return false;
  ps.skip_ws();
  if (!ps.at_end())
    return false;
  if (root->kind != json::Val::OBJ)
    return false;
  for (const auto &pv : root->obj) {
    ...
    if (api_val && api_val->kind == json::Val::STR) {
      ...
      if (!a.empty() && !pr.api_to_id.count(a))
        pr.api_to_id[a] = provider;
    }
    const json::Val *models_obj = pv.second->find("models");
    if (!models_obj || models_obj->kind != json::Val::OBJ)
      continue;
...
static bool load_pricing(const string &path, Pricing &pr,
                         bool &missing_default_path) {
  ...
  if (!closed || buf.empty() || !pricing_from_json(buf, pr))
    return false;
  pr.path = path;
  pr.loaded = true;
  return true;
...
static bool rate_for_key(const Pricing &pr, const string &key, Rate &r) {
  ...
  for (const auto &cand : cands)
    if (cand.second.in > 0 || cand.second.out > 0)
      if (!best || cand.second.in < best->second.in)
        best = &cand;
  if (best)
    pick = best;
  r.in = pick->second.in;
...
static bool resolve_cost(const Pricing &pr, const string &model, Rate &r) {
  if (model.empty() || !pr.loaded)
    return false;
  if (rate_for_key(pr, model, r))
    return true;
...
static bool resolve_cost_for_provider(const Pricing &pr, const string &model,
                                      const string &pid, ModelCost &mc,
                                      string *model_key_out) {
  auto scoped = [&](const string &key, ModelCost &out) -> bool {
    ...
    for (auto &kv : pr.by_id) {
      ...
      if (sep == string::npos)
        continue;
      if (kv.first.substr(0, sep) != pid)
        continue;
      if (iequals(kv.first.substr(sep + 1), key)) {
        out = kv.second;
        return true;
      }
    }
  ...
  if (slash != string::npos) {
    if (scoped(model.substr(slash + 1), mc)) {
      if (model_key_out)
        *model_key_out = model.substr(slash + 1);
      return true;
    }
  } else {
    for (const auto &kv : pr.key_provider) { // try namespaced keys
      size_t sep = kv.first.find('\x1f');
      if (sep == string::npos || kv.first.substr(sep + 1) != pid)
        continue;
...
// Format a USD amount: enough precision for sub-cent values, trimmed for big
// ones.
...
static vector<std::pair<string, long long>>
sorted_pairs(const map<string, long long> &m) {
  vector<std::pair<string, long long>> v(m.begin(), m.end());
  std::stable_sort(v.begin(), v.end(),
                   [](const std::pair<string, long long> &a,
                      const std::pair<string, long long> &b) {
...
int main(int argc, char **argv) {
  // Arg parsing: <log-file>... <output.md> [--cost /path/to/modelsdev.json]
  // [--no-cost]
  vector<const char *> pos;
  bool no_cost = false;
  const char *cost_flag = nullptr;
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--no-cost") == 0) {
      no_cost = true;
      continue;
    }
  ...
  if (pos.size() < 2) {
    fprintf(stderr,
            "usage: %s <log-file>... <output.md> [--cost "
            "/path/to/modelsdev.json] [--no-cost]\n",
            argv[0]);
    return 2;
  }
  ...
  // NOTE: default path is NOT auto-created; see error message below when
  // missing.
  ...
  if (!no_cost && !pricing_path) {
    ...
    if (home && *home) {
      snprintf(cand, sizeof cand, "%s/.agentty/cache/modelsdev.json", home);
      pricing_path = cand;
    } else if (prof && *prof) {
      snprintf(cand, sizeof cand, "%s/.agentty/cache/modelsdev.json", prof);
      pricing_path = cand;
    } else {
      pricing_path = "~/.agentty/cache/modelsdev.json";
    }
  ...
  for (size_t i = 0; i < n_inputs; i++)
    bootstrap_roles(s, pos[i]);
  for (size_t i = 0; i < n_inputs; i++) {
    if (!process_file(s, pos[i]))
      return 1;
  ...
  if (pricing_enabled) {
    ...
    for (Turn &t : s.turn_list) {
      ...
      string prov_disp =
          t.provider.empty()
              ...
              : norm_api(t.provider);
      if (t.provider.empty() && !pid.empty())
        prov_disp = pid;
      string mkey = t.model;
      bool exact = true;
      ModelCost mc;
      bool local =
          is_local_api(prov_disp) || is_local_api(norm_api(t.api_base));
      if (!local) {
        bool pinned_priced = false, listed_at_provider = false;
        if (!pid.empty()) {
          pinned_priced =
              resolve_cost_for_provider(pricing, t.model, pid, mc, &mkey);
          if (!pinned_priced) {
        ...
        if (pid.empty()) {
          ...
          for (auto &kv : pricing.key_provider) {
            ...
            if (sep == string::npos)
              continue;
            if (!iequals(kv.first.substr(0, sep), mkey) &&
                kv.first.substr(0, sep) != mkey)
              continue;
    ...
    for (auto &kv : cost_acc) {
      ...
      size_t sep2 =
          kv.first.find('\x1f', sep1 == string::npos ? sep1 : sep1 + 1);
      kv.second.disp_provider = kv.first.substr(0, sep1);
      kv.second.disp_model = kv.first.substr(sep1 + 1, sep2 - sep1 - 1);
      kv.second.disp_model_key = sep2 == string::npos
                                     ? kv.second.disp_model
  ...
  if (pricing_missing || pricing_bad) {
    fprintf(
        ...
        "    $AGENTTY_HOME/cache/modelsdev.json   (default: "
        "~/.agentty/cache/modelsdev.json)\n"
        "\n"
        "Fix it by pointing agentty-stats at the file, then re-run:\n"
        "    agentty-stats <log-files>... <output.md> --cost "
        "/path/to/modelsdev.json\n"
  ...
  o << "| Time span | " << s.first_ts << " → " << s.last_ts << " ("
    << human_ms(s.wall_max_ms - s.wall_min_ms) << ") |\n";
  o << "| Models used | ";
  {
    bool first = true;
  ...
  o << "| Total prompt tokens | " << human_tokens(s.total_prompt) << " ("
    << s.total_prompt << ") |\n";
  o << "| Total completion tokens | " << human_tokens(s.total_completion)
    << " (" << s.total_completion << ") |\n";
    ...
    << " |\n";
  {
    double grand = 0;
    for (auto &kv : cost_acc)
      grand += kv.second.total_usd;
    o << "| Estimated cost | "
      << (cost_acc.empty()
              ? "n/a"
              : usd_fmt(grand) +
                    (pricing_enabled ? "" : " (pricing unavailable)"))
      << " |\n";
  }
  o << "| Wire chunks | " << s.total_chunks << " ("
    << human_bytes(s.total_chunk_bytes) << ") |\n";
  o << "| Request bytes | " << human_bytes(s.total_request_bytes) << " |\n";
  o << "| Tool calls | " << ([](const map<string, ToolStat> &t) {
    long long n = 0;
    for (auto &kv : t)
      n += kv.second.calls;
    return n;
    ...
    << " |\n";
  o << "| Thread saves | " << s.thread_saves << " (" << s.thread_messages
    << " messages) |\n";
  o << "| Models loaded | ";
  if (s.source_files.empty())
    o << "0";
  else {
    bool first = true;
    for (auto &sf : s.source_files) {
      ...
      o << base << "=";
      if (sf.models_loaded >= 0)
        o << sf.models_loaded;
      else
        o << "n/a"; // session continuation: no models.loaded event in this file
    }
  }
  o << " |\n";
  ...
  o << " |\n";
  o << "| Error-level events | " << s.level_count["E"] - s.auth_events << " ("
    << s.auth_events << " routine auth) |\n\n";

  // 2. Log volume by level / component / event
  o << "## 2. Log volume\n\n";
  o << "### By level\n\n| Level | Count |\n|---|---|\n";
  for (auto &kv : sorted_pairs(s.level_count))
    o << "| " << kv.first << " | " << kv.second << " |\n";
  o << "\n### By component\n\n| Component | Count |\n|---|---|\n";
  for (auto &kv : sorted_pairs(s.component_count))
    o << "| " << kv.first << " | " << kv.second << " |\n";
  o << "\n### By event\n\n| Event | Count |\n|---|---|\n";
  for (auto &kv : sorted_pairs(s.event_count))
    o << "| " << kv.first << " | " << kv.second << " |\n";
  o << "\n";
  ...
  // fall back to the previous resolved role).
  {
    ...
    for (Turn &t : s.turn_list) {
      ...
      if (t.role_source == 1)
        src_model++;
      else if (t.role_source == 2)
        src_prev++;
      else if (t.role_inferred.empty() || t.role_inferred == "none")
        src_unres++;
    }
    ...
    for (auto &kv : sorted_pairs(role_res))
      o << "| " << kv.first << " | " << kv.second << " | "
        << (total_turns_n
                ? pct_fmt(100.0 * (double)kv.second / (double)total_turns_n)
                : "-")
        << " |\n";
    ...
    for (auto &kv : role_res_prompt)
      tok_keys.push_back(kv.first);
    std::sort(tok_keys.begin(), tok_keys.end(),
              [&](const string &a, const string &b) {
                long long ta = role_res_prompt[a] + role_res_compl[a],
                          tb = role_res_prompt[b] + role_res_compl[b];
    ...
    for (auto &k : tok_keys) {
      ...
      o << "| " << k << " | " << human_tokens(role_res_prompt[k]) << " | "
        ...
        << " | "
        << (tot_prompt + tot_compl ? pct_fmt(100.0 * (double)tot /
                                             (double)(tot_prompt + tot_compl))
                                   : "-")
        << " |\n";
    }
  ...
  // 4. Per-model usage
  o << "## 4. Per-model usage\n\n";
  o << "| Model | Turns | Prompt tok | Completion tok | Total tok | Req bytes "
       "| Chunk bytes | Chunks | Retried | Errors |\n";
  o << "|---|---|---|---|---|---|---|---|---|---|\n";
  {
    vector<const std::pair<const string, ModelStat> *> model_rows;
    for (const auto &kv : s.models)
      model_rows.push_back(&kv);
    std::stable_sort(model_rows.begin(), model_rows.end(),
                     [](const std::pair<const string, ModelStat> *a,
                       ...
                       if (a->second.turns != b->second.turns)
                         return a->second.turns > b->second.turns;
                       return a->first < b->first;
                     });
    for (const auto *kv : model_rows) {
      ...
      o << "| " << name << " | " << ms.turns << " | "
        ...
        << human_bytes(ms.request_bytes) << " | " << human_bytes(ms.chunk_bytes)
        << " | " << ms.chunks << " | " << ms.retried_turns << " | " << ms.errors
        << " |\n";
    }
  }
  o << "\n";
  ...
  o << "## 4b. Cost (USD)\n\n";
  o << "*Prices from [models.dev](https://models.dev), loaded from `"
    << pricing_path
    << "`; all amounts are United States Dollars (USD). Cost = prompt/1M × "
       "input-rate + completion/1M × output-rate.*\n\n";
  if (no_cost) {
    o << "*Cost reporting disabled (`--no-cost`).*\n\n";
  } else if (cost_acc.empty()) {
    o << "*No priced models: none of the models in the logs could be matched "
         "to a models.dev entry.*\n\n";
  } else {
    o << "| Provider | Model | $/1M in | Input cost | $/1M out | Output cost | "
         "Turns | Total cost |\n";
    o << "|---|---|---|---|---|---|---|---|\n";
    vector<const std::pair<const string, CostAcc> *> cost_rows;
    for (const auto &kv : cost_acc)
      cost_rows.push_back(&kv);
    ...
    for (const auto *kv : cost_rows) {
      ...
      o << "| " << a.disp_provider << " | " << a.disp_model << " | " << rate_in
        << " | " << usd_fmt(a.input_usd) << " | " << rate_out << " | "
        << usd_fmt(a.output_usd) << " | " << a.turns << " | "
        << usd_fmt(a.total_usd) << " |\n";
    }
    ...
    for (auto &kv : s.models) {
      ...
      for (auto &c : cost_acc)
        if (c.second.disp_model == kv.first) {
          any_priced = true;
          break;
        }
      if (any_priced)
      ...
      o << "*No pricing data for model `" << kv.first << "` ("
        << kv.second.turns << " turns, "
        << human_tokens(kv.second.prompt_tokens + kv.second.completion_tokens)
        << " tokens) — excluded from cost totals.*\n";
    }
    if (unpriced_turns)
      o << "\n";
    o << "*Note: estimates use input/output token counts from the logs; "
         "cache-read/cache-write tokens are not logged and are therefore not "
  ...
  o << "|---|---|---|---|---|---|---|\n";
  {
    ...
    for (const auto &kv : s.tools)
      tool_rows.push_back(&kv);
    ...
    for (const auto *kv : tool_rows) {
      ...
      o << "| " << kv->first << " | " << ts.calls << " | "
        ...
        << human_ms(ts.max_ms) << " | " << ts.ok << " | " << ts.err << " |\n";
    }
  }
  o << "\n";
  ...
  o << "## 6. Wire / streaming\n\n";
  o << "### Stop reasons\n\n| Stop | Count |\n|---|---|\n";
  for (auto &kv : sorted_pairs(s.stop_count))
    o << "| " << kv.first << " | " << kv.second << " |\n";
  o << "\n### HTTP status\n\n| Status | Count |\n|---|---|\n";
  for (auto &kv : sorted_pairs(s.status_count))
    o << "| " << kv.first << " | " << kv.second << " |\n";
  o << "\n### Hosts\n\n| Host | Count |\n|---|---|\n";
  for (auto &kv : sorted_pairs(s.host_count))
    o << "| " << kv.first << " | " << kv.second << " |\n";
  o << "\n### Providers\n\n| Provider | Count |\n|---|---|\n";
  for (auto &kv : sorted_pairs(s.provider_count))
    o << "| " << kv.first << " | " << kv.second << " |\n";
  o << "\n";
  ...
  if (!any_fail) {
    ...
    for (auto &kv : s.http_error_status)
      http_errs += kv.second;
    ...
    if (!s.http_error_status.empty()) {
      o << "### HTTP error status\n\n| Status | Count |\n|---|---|\n";
      for (auto &kv : sorted_pairs(s.http_error_status))
        o << "| " << kv.first << " | " << kv.second << " |\n";
      o << "\n";
    }
    ...
    if (!s.stream_error_class.empty()) {
      ...
      for (auto &kv : sorted_pairs(s.stream_error_class))
        o << "| " << kv.first << " | " << kv.second << " |\n";
      o << "\n";
    }
    if (!s.error_turn_samples.empty()) {
      o << "**Samples:**\n\n```\n";
      for (auto &str : s.error_turn_samples)
        o << str << "\n";
      o << "```\n\n";
    }
  }
  ...
  o << "| # | Model | Complexity | Stop | Retry | Prompt | Completion | Cost | "
  ...
  o << "|---|---|---|---|---|---|---|---|---|---|---|\n";
  {
    ...
    for (Turn &t : s.turn_list)
      ordered.push_back(&t);
    std::sort(ordered.begin(), ordered.end(), [](const Turn *a, const Turn *b) {
      return a->wall_start_ms < b->wall_start_ms;
    });
    int i = 1;
    for (const Turn *t : ordered) {
      long long dur = (t->has_end && t->wall_end_ms >= t->wall_start_ms)
                          ? (t->wall_end_ms - t->wall_start_ms)
                          : 0;
      string turn_cost = "-";
      if (pricing_enabled && t->has_cost)
        ...
        turn_cost = "n/a";
      o << "| " << i++ << " | " << (t->model.empty() ? "-" : t->model) << " | "
        << (t->complexity.empty() ? "-" : t->complexity) << " | "
        << (t->stop.empty() ? "-" : t->stop) << " | " << t->retry << " | "
        << human_tokens(t->prompt_tokens) << " | "
        << human_tokens(t->completion_tokens) << " | " << turn_cost << " | "
        << t->chunks << " | " << human_bytes(t->chunk_bytes) << " | "
        << human_ms(dur) << " |\n";
    }
  }
  o << "\n";
  ...
  if (s.err_events.empty()) {
    ...
    o << "| Event | Count |\n|---|---|\n";
    for (auto &kv : sorted_pairs(s.err_events))
      o << "| " << kv.first << " | " << kv.second << " |\n";
    o << "\n**Samples:**\n\n```\n";
    for (auto &str : s.err_samples)
      o << str << "\n";
    o << "```\n\n";
  }
  ...
  o << "- Avg messages per save: "
    << (s.thread_saves ? s.thread_messages / s.thread_saves : 0) << "\n\n";
  ...
  fprintf(stderr, "wrote %s (%zu bytes, %zu turns, %zu file%s)\n", out_path,
          o.str().size(), s.turn_list.size(), n_inputs,
          n_inputs == 1 ? "" : "s");
  return 0;
}
